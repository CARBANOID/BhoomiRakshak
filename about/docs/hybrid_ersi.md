...
// ================================================================
// Trained on Esri 10m Annual Land Cover (TS) — latest year available.
// Used to fill pixels where DW confidence < DW_MIN_CONFIDENCE.
// Same 18-band input as baseline image (optical + terrain + SAR).
// ================================================================

// Fetch the latest available year of Esri Land Cover (TS)
...
// Remap Esri schema to Bhoomi 7-class schema:
// Esri: 1:Water, 2:Trees, 4:FloodedVeg, 5:Crops, 7:Built, 8:Bare, 9:Snow, 11:Rangeland
// Bhoomi: 1:Forest, 2:Shrub, 3:Crop, 4:Urban, 5:Barren, 6:Water, 7:Snow
var labels = esri.remap(
  [1, 2, 4, 5, 7, 8, 9, 11],
  [6, 1, 2, 3, 4, 5, 7,  2]
).rename('label').toInt();
...
// WITH rolling window matching Python config (5yr baseline, 2yr lag):
...
RAW THREAT DETECTIONS (km2)
  Deforestation:   33.9
  Vegetation Loss: 17.8
  Water Degrad:    1.9
  Urban Expansion: 32.3
  Soil Degrad:     10.7
  Agri Loss:       17.4
  Poaching Proxy:  0.4
---------------------------------------------
FILTERED THREAT AREAS (km2)
  Deforestation:   10.11
  Vegetation Loss: 8.60
  Water Degrad:    1.90
  Urban Expansion: 10.12
  Soil Degrad:     3.44
  Agri Loss:       12.89
  Poaching Proxy:  0.01
---------------------------------------------
AOI-NORMALIZED THREATS (% of AOI)
  Deforestation:   0.330%
  Vegetation Loss: 0.280%
  Water Degrad:    0.062%
  Urban Expansion: 0.330%
  Soil Degrad:     0.112%
  Agri Loss:       0.420%
  Poaching Proxy:  0.000%
---------------------------------------------
ACTIVE ALERTS (2/7)
  WATER: 1.90 km2 | 0.062% AOI | LOW
  AGRI: 12.89 km2 | 0.420% AOI | LOW