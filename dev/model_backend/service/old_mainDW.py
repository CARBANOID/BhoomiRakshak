...
def _run_dynamic_world(payload: MonitoringRunRequest, overrides: dict[str, float]) -> MonitoringRunResponse:
    ...
    def get_dynamic_world_class_composite(start_date: Any, end_date: Any) -> DynamicWorldArtifacts:
        ...
        fallback_cls = ee.Image.constant(2).rename("cls").toInt().clip(aoi)
        ...
        final_cls_conf = final_cls_no_mask.updateMask(
            final_max_prob.gte(_safe_number(config.get("DW_MIN_CONFIDENCE")))
        )

        cls = final_cls_no_mask.where(final_cls_conf.mask(), final_cls_conf).clip(aoi)
    ...
    dw_baseline = get_dynamic_world_class_composite(baseline_start_date, baseline_end_date_exclusive)
    ...
    dw_current = get_dynamic_world_class_composite(dw_current_start, dw_current_end)
