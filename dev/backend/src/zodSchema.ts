...
const jsonValueSchema: zod.ZodType<JsonValue> = zod.lazy(() =>
    zod.union([
        zod.string(),
        zod.number(),
        zod.boolean(),
...
const thresholdProfileModes = ["strict", "balanced", "permissive", "custom"] as const;
...
const thresholdConfigAllowedKeySet = new Set<string>(thresholdConfigAllowedKeys);
...
const zodThresholdConfigNonEmptySchema = zodThresholdConfigSchema.refine(
    (config) => Object.keys(config).length > 0,
    "config must include at least one threshold key"
);
