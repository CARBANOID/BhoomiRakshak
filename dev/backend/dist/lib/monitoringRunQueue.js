...
import { buildFailureMessage, computeRetryDelayMs, inspectRetryableError, resolveNextRunAt } from "./monitoringQueuePolicy.js";
...
const retryAttemptsByRunId = new Map();
const queueMetrics = {
    claimed: 0,
    succeeded: 0,
    failed: 0,
...
function logQueueEvent(level, event, fields = {}) {
    const payload = {
...
async function persistRunSuccess(run, output) {
    ...
    const suppressedAlertRows = finalAlertRows.filter(newAlert => {
        ...
        const isMoreLethal = (newAlert.areaKm2 > existing.areaKm2 * 1.1) ||
...
async function scheduleRunRetry(run, error, attempt) {
...
async function processRun(run) {
    ...
    try {
        ...
        const statusCodeFields = typeof retryInspection.statusCode === "number" ? { statusCode: retryInspection.statusCode } : {};
        ...
        await markRunFailed(run.id, buildFailureMessage(error, `retryable=${retryInspection.retryable ? "true" : "false"}; reason=${retryInspection.reason}; attempt=${attempt}`));
    }
}
async function claimAndQueueOneDueSchedule(now) {
    ...
    const queued = await prismaClient.$transaction(async (tx) => {
        ...
        if (typeof candidate.configSnapshot !== "undefined" && candidate.configSnapshot !== null) {
            runData.configSnapshot = candidate.configSnapshot;
        }
...
async function drainQueue(trigger) {
    if (draining) {
...
async function drainDueSchedules(trigger) {
    if (scheduleDraining) {
...
export function notifyRunQueued() {
    void drainQueue("run-queued");
}
export function startMonitoringRunQueue() {
    if (queueTimer) {
        return;
    }
...
export function stopMonitoringRunQueue() {
...
//# sourceMappingURL=monitoringRunQueue.js.map